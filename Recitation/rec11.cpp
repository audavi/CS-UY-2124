...
Node* reverse(const Node* hp) {
    Node* newHeadPtr = nullptr;
    while (hp != nullptr) {
        Node* newNode = new Node(hp->data); //newNode =  6 -- newNode = 3 -- newNode = 2 -- newNode = 5
        newNode->next = newHeadPtr; // next: nullptr -- next: 6 -- next: 3 -- next: 2
        newHeadPtr = newNode; // newHeadPtr ->6->nullptr -- newHeadPtr->3->6->nullptr -- newHeadPtr->2->3->6->nullptr --newHeadPtr->5->2->3->6->nullptr
        hp = hp->next; // move to 3 -- move to 2 -- move to 5 -- move to nullptr
    }
    return newHeadPtr;
}
...
void reverseInPlace(Node*& hp) {
    if (hp == nullptr) {
        ...
        Node* prevNode = nullptr; // becomes the tail
        Node* currentNode = hp; // becomes the head
        while (currentNode != nullptr) { // curr = 6 -- curr = 3 -- ..
            Node* next = currentNode->next; //currentNode->next = 3 -- currentNode->next = 2 -- ..
            currentNode->next = prevNode; // set 6->next to nullptr -- set 3->next = 6 -- ..
            prevNode = currentNode; // set prevNode to 6 -- set prevNode to 3 -- ..
            currentNode = next; // currentNode becomes 3 -- currentNode becomes 2 -- ...
        }
        hp = prevNode;
    }
}

bool isSublist_helper(const Node* pattern, const Node* target) {
    while (pattern && target) { // go through pattern and target
        if (pattern->data != target->data) {
            return false; // not a sublist
        }
        pattern = pattern->next; // move up a node
        target = target->next; // move up a node
    }

    return !pattern; // check if pattern is nullptr (patten has been checked completely)
}
...
const Node* isSublist(const Node* pattern, const Node* target) {
    while(target) { // go through target list
        if (isSublist_helper(pattern , target)) { // check if the node matches
            return target; // position where a match was found
        }
        target =  target->next; // move onto next node in target list if no match found
    }
    return nullptr; // no match after going through the entire list
}
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    while (listA) { //compare each listA node to listB
        const Node* listBCopy = listB;
        while (listBCopy) {
            if (listBCopy != listA ) {
                listBCopy = listBCopy->next;
            } else {
                return listBCopy; // first shared node
            }
        }
        listA = listA->next; // continue looking for shared node
    }
    return nullptr;
}
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB) {
    ...
    while(listB) {
        if (set.find(listB) != set.end()) {
            //if a node in listB is in the set, there is a shared node
            return listB;
        }
        listB = listB->next;
    }
